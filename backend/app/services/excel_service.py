...
# Only improves structural header scoring; this is NOT a business-field whitelist.
...
EXACT_UNIT_HINTS = {"nps数值": "in", "毫米参考": "mm"}
...
def _normalize_unit(value) -> str | None:
    ...
    if not text or text.lower() in {"nan", "none", "-", "/", "待定", "待确认"}:
        return None
    aliases = {
        "毫米": "mm", "millimeter": "mm", "millimetre": "mm", "英寸": "in", "inch": "in", "inches": "in",
        "mpa": "MPa", "kpa": "kPa", "pa": "Pa", "n": "N", "kn": "kN", "nm": "N·m", "n·m": "N·m",
        "°c": "℃", "c": "℃",
    }
...
def _infer_units(frame: pd.DataFrame, columns: list[dict]) -> list[dict]:
    result = [dict(item) for item in columns]
    index_by_source = {str(item.get("source_name")): idx for idx, item in enumerate(result)}

    for item in result:
        item["unit"] = item.get("unit") or _unit_from_header(item.get("field_name") or item.get("source_name")) or ""

    for pos, col in enumerate(list(frame.columns)):
        if str(col).strip().lower() not in UNIT_COLUMN_NAMES or pos <= 0:
            continue
        units = [_normalize_unit(v) for v in frame[col].head(100).tolist()]
        ...
        if target_idx is None:
            continue
        previous_name = str(result[target_idx].get("field_name") or previous).strip().lower()
        if previous_name in {"取值", "值", "示例值", "value", "sample value"}:
            continue
...
def preview_payload(path: Path, sheet_name: str, header_row: int, limit: int = 8) -> dict:
    ...
    columns = _infer_units(frame, columns)
    ...
    return {
        ...
        "detected_header_row": NO_HEADER if analysis.get("header_row") is None else int(analysis.get("header_row")),
    }
