...
RESULT_FIELDS = [
    ("SECTION", "分类", "text"), ("PARAMETER", "参数", "text"), ("VALUE", "取值", "text"),
    ("UNIT", "单位", "text"), ("SOURCE", "来源", "text"), ("STATUS", "状态", "text"),
    ("CALCULATION", "计算/规则", "text"), ("PART_OUTPUT", "零件输出", "text"),
    ("PART_TARGET", "零件写入位置", "text"), ("ASSEMBLY_OUTPUT", "装配输出", "text"),
    ("ASSEMBLY_TARGET", "装配写入位置", "text"), ("TARGET_FILE", "目标文件", "text"),
    ("VARIABLE", "SW变量", "text"), ("FEATURE", "Feature/Mate", "text"), ("SOURCE_TYPE", "来源类型", "text"),
]
INPUT_LABELS = [
    ("valve_structure", "阀门结构", "-"), ("nps", "公称口径", "-"), ("pressure_class", "压力等级", "-"),
    ("bore_type", "通径形式", "-"), ("end_connection", "端部连接", "-"), ("body_material", "阀体材料", "-"),
    ("ball_material_group", "球体材料组", "-"), ("seat_material", "阀座材料", "-"),
    ("design_pressure", "设计压力", "MPa"), ("design_temperature", "设计温度", "℃"),
    ("leakage_level", "泄漏等级", "-"), ("medium", "介质", "-"),
]
ENGINEERING_LABELS = [
    ("seat_angle", "座角", "°"), ("operating_torque", "操作扭矩", "N·m"),
    ("allowable_shear", "阀杆许用剪应力", "MPa"), ("seat_preload", "座轴向预压", "mm"),
    ("stem_engage_factor", "阀杆啮合系数k", "-"), ("body_bolt_circle", "体螺栓节圆", "mm"),
]
...
def ensure_result_library(db: Session) -> KnowledgeBase:
    ...
    if kb:
        return kb
    kb = KnowledgeBase(
        name=RESULT_KB_NAME, code=RESULT_KB_CODE, type="result",
        description="保存参数工作台每一次生成的输入快照、计算链、零件参数、装配参数和SolidWorks映射。",
        accent="blue", enabled=True,
    )
...
def result_row(section: str, parameter: str, value, unit: str = "-", source: str = "-", status: str = "已保存", **extra) -> dict:
    row = {
        "SECTION": section, "PARAMETER": parameter, "VALUE": normalize_value(value),
        "UNIT": normalize_value(unit), "SOURCE": normalize_value(source), "STATUS": normalize_value(status),
    }
    row.update({key: normalize_value(val) for key, val in extra.items() if val is not None})
    return row
...
def save_workbench_result(body: WorkbenchSaveRequest, user: User = Depends(require_user), db: Session = Depends(get_db)):
    ...
    dataset = Dataset(
        knowledge_base_id=kb.id, name=dataset_name[:180], category="参数生成记录",
        standard_no=f"{nps} / {pressure} / {leakage}", source_file_name=f"参数生成工作台 · {user.display_name}",
        sheet_name=None, record_count=0, field_count=len(RESULT_FIELDS),
    )
    db.add(dataset); db.flush()
    for order_no, (code, field_name, data_type) in enumerate(RESULT_FIELDS, start=1):
        db.add(DatasetField(
            dataset_id=dataset.id, source_name=field_name, field_code=code, field_name=field_name,
            data_type=data_type, unit=None, order_no=order_no, searchable=True,
            filterable=code in {"SECTION", "PARAMETER", "SOURCE", "STATUS", "SOURCE_TYPE"},
        ))

    rows = [
        result_row("保存信息", "保存人", user.display_name, "-", "系统", "已保存"),
        result_row("保存信息", "保存时间", now.strftime("%Y-%m-%d %H:%M:%S"), "-", "系统", "已保存"),
    ]
    rows += [result_row("输入条件", label, snapshot.get(key), unit, "用户输入", "已保存") for key, label, unit in INPUT_LABELS]
    engineering = snapshot.get("engineering") or {}
    rows += [result_row("企业计算条件", label, engineering.get(key), unit, "用户/企业规则输入", "待企业确认") for key, label, unit in ENGINEERING_LABELS]
    ...
    for item in body.calculation_results:
        rows.append(result_row(
            "计算过程", f"{item.step} {item.title}", item.calculation or "-", "-", item.source or "-", item.status or "待确认",
            CALCULATION=item.calculation, PART_OUTPUT=item.part_output, PART_TARGET=item.part_target,
            ASSEMBLY_OUTPUT=item.assembly_output, ASSEMBLY_TARGET=item.assembly_target, SOURCE_TYPE=item.source_type,
        ))
    for item in body.sw_mappings:
        rows.append(result_row(
            "SolidWorks映射", item.parameter, item.value, item.unit or "-", item.target_file or "-", item.status or "待确认",
            TARGET_FILE=item.target_file, VARIABLE=item.variable, FEATURE=item.feature,
        ))

    db.bulk_save_objects([DatasetRecord(dataset_id=dataset.id, data=row) for row in rows])
    dataset.record_count = len(rows); kb.updated_at = now
...
def _group_result(rows: list[dict]) -> dict:
    grouped = {"保存信息": [], "输入条件": [], "企业计算条件": [], "零件几何参数": [], "装配/接口参数": [], "计算过程": [], "SolidWorks映射": []}
    for row in rows:
        section = row.get("SECTION", "其他")
        item = {"name": row.get("PARAMETER", "-"), "value": row.get("VALUE", "-"), "unit": row.get("UNIT", "-"), "source": row.get("SOURCE", "-"), "status": row.get("STATUS", "-")}
        if section == "计算过程":
            item.update({
                "calculation": row.get("CALCULATION", row.get("VALUE", "-")), "part_output": row.get("PART_OUTPUT", "-"),
                "part_target": row.get("PART_TARGET", "-"), "assembly_output": row.get("ASSEMBLY_OUTPUT", "-"),
                "assembly_target": row.get("ASSEMBLY_TARGET", "-"), "source_type": row.get("SOURCE_TYPE", "-"),
            })
        if section == "SolidWorks映射":
            item.update({"parameter": row.get("PARAMETER", "-"), "target_file": row.get("TARGET_FILE", row.get("SOURCE", "-")), "variable": row.get("VARIABLE", "-"), "feature": row.get("FEATURE", "-")})
        grouped.setdefault(section, []).append(item)
    snapshot = {x["name"]: x["value"] for x in grouped.get("输入条件", [])}
    saved = grouped.get("保存信息", [])
    saved_at = next((x["value"] for x in saved if x["name"] == "保存时间"), None)
    return {
        "snapshot": snapshot, "engineering_inputs": grouped.get("企业计算条件", []), "saved_at": saved_at,
        "geometry_results": grouped.get("零件几何参数", []), "assembly_results": grouped.get("装配/接口参数", []),
        "calculation_results": grouped.get("计算过程", []), "sw_mappings": grouped.get("SolidWorks映射", []), "save_info": saved,
    }
...
def list_workbench_results(keyword: str = "", nps: str = "", pressure_class: str = "", leakage_level: str = "", sort: str = "newest", db: Session = Depends(get_db)):
    ...
    for dataset in datasets:
        ...
        rows.append({
            ...
            "record_count": dataset.record_count, "geometry_count": len(grouped["geometry_results"]), "assembly_count": len(grouped["assembly_results"]),
            "calculation_count": len(grouped["calculation_results"]), "sw_mapping_count": len(grouped["sw_mappings"]), "pending_count": pending,
        })
...
def _api6d_virtual_link() -> dict:
    ...
    rows = [{"_id": f"api6d-{idx}", **item} for idx, item in enumerate(API6D_TOP_MOUNTED_BALL_STRUCTURE, start=1)]
    return {
        "virtual": True, "standard_code": "API 6D", "part_name": "球阀（顶装固定式）",
        ...
        "dataset_id": None, "dataset_exists": True, "source_document": "API 6D-2021 中文版 石油和天然气工业管线阀门",
        "fields": field_payload, "rows": rows,
    }
...
def standard_detail(standard_id: int, db: Session = Depends(get_db)):
    if standard_id < 1 or standard_id > len(STANDARD_REGISTRY):
    ...
    for link in links:
        ...
        if dataset:
            ...
            records = db.scalars(select(DatasetRecord).where(DatasetRecord.dataset_id == dataset.id).order_by(DatasetRecord.id.asc()).limit(80)).all(); codes = [f.field_code for f in selected]
