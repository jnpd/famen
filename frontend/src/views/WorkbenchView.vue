  ...
  <div class="workbench-page" v-loading="loading">
    ...
    <div class="workbench-top">
      <section class="panel input-panel">
        <div class="panel-title"><div><b>① 输入设计条件</b><span>用户只输入设计边界，系统负责查表和汇总。</span></div><el-tag effect="plain">输入</el-tag></div>
        <el-form label-position="top" class="input-grid">
          ...
          <el-form-item label="介质" class="span-two"><el-input v-model="form.medium" placeholder="例如 天然气" /></el-form-item>
        </el-form>
        <div class="action-row"><el-button @click="resetForm">恢复示例</el-button><el-button type="primary" :loading="generating" @click="generate"><el-icon><Operation /></el-icon>生成综合参数</el-button></div>
      </section>
...
const defaults = { valve_structure:'固定球阀', nps:'NPS12', pressure_class:'CL600', bore_type:'全通径', end_connection:'RF法兰', body_material:'A105', ball_material_group:'碳钢/合金钢球', seat_material:'PEEK', design_pressure:'2', design_temperature:'-29~120', medium:'天然气' }
...
async function getDetail(name) {
  ...
  if (!ds) { if (!missingDatasets.value.includes(name)) missingDatasets.value.push(name); return null }
  if (detailCache.has(ds.id)) return detailCache.get(ds.id)
  const { data } = await http.get(`/datasets/${ds.id}`); detailCache.set(ds.id, data); return data
}
...
async function query(name, filters={}, pageSize=200) {
...
function tagType(status) { if (/可用|已录入|通过/.test(status || '')) return 'success'; if (/待/.test(status || '')) return 'warning'; if (/缺失|错误/.test(status || '')) return 'danger'; return 'info' }

async function generate() {
  generating.value = true; generated.value = false; missingDatasets.value = []
  try {
  ...
  } catch(e) { ElMessage.error(`综合参数生成失败：${e.message}`) } finally { generating.value = false }
}

async function loadValues(dsName, fieldName) { const x=await query(dsName,{}); return [...new Set(x.rows.map(r=>value(x.detail,r,fieldName)).filter(Boolean).map(String))] }
async function loadOptions() {
  const [a,b,c,d,e] = await Promise.all([loadValues('口径基础表','公称口径'),loadValues('压力等级表','压力等级'),loadValues('通径表','通径形式'),loadValues('结构长度表','端部连接'),loadValues('球体直径表','球体材料组')])
  options.nps=a; options.pressure=b; options.boreType=c; options.endConnection=d; options.ballMaterial=e
}
